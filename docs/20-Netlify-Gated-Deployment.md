...
Netlify deploys the live site automatically whenever a commit is pushed to `main`. GitHub Actions validates the same commit, but it does not trigger or block the Netlify deployment.
...
2. In **Project configuration -> Environment variables**, add encrypted production values:
   ...
   - `NODE_ENV=production`.
   - Use the **Production** deploy context. On plans that offer scopes, include both **Functions** and **Builds**. Netlify Functions cannot read a value that exists only for a build.
...
Then trigger a new production deploy in Netlify and verify `/api/health` returns `{"status":"ok"}`. Only then sign in at `/login` with the owner email and password you chose. Do not run `db:seed` again after the first production seed.
